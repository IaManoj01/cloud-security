...
public class FileService {
    ...
    public ResponseEntity<Object> encryptAndSave(MultipartFile file, String username){
        try {
            ...
            byte[] encryptedBytes = HybridCryptoUtil.packHybridData(encryptedData);  // ✅ CORRECT


            File outFile = new File("uploads/" + file.getOriginalFilename() + ".enc");
    ...
    public ResponseEntity<byte[]> decryptAndDownload(Long id) {
        try {
            ...
            HybridEncryptedData data = HybridCryptoUtil.unpackHybridData(encrypted); // ✅ Unpack it
